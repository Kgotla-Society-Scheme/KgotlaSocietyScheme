...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      ...
      modifiedDate: payment.modifiedDate && payment.modifiedDate.isValid() ? payment.modifiedDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        ...
        payment.modifiedDate = payment.modifiedDate ? moment(payment.modifiedDate) : undefined;
      });
    }
    return res;
  }
